...
c-------------------------------------------------------------------
      subroutine elec(buffer,deltax,n_graf,nstep)
      use garcha_mod
c
      implicit real*8 (a-h,o-z)
c auxiliar quantities
      dimension Q(3),xi(3)
      real*8, dimension (:), ALLOCATABLE :: pote,densidad
c      parameter (deltax=.5D0)      
c      parameter (buffer=5.D0)
      parameter (Ar0=0.529177D0)
      real*8 VVV,buffer,deltax
      integer n_graf,nstep
      CHARACTER(LEN=100) :: filename_dens,filename_field
c
c      deltax=.5
      rmax=3.
      do 1 l=1,3
...
c
      ...
      write(*,*) 'deltax', deltax
      write(*,*) 'grilla', NX,NY,NZ
      write(*,*) 'minimos',Xmin,Ymin,Zmin
      write(*,*) 'maximos',Xmax,Ymax,Zmax
      allocate(pote(Ntot),densidad(Ntot))
...
c       else
       ...
       icont=0
       if(n_graf.eq.1.or.n_graf.eq.3) then
       ntotal=0
       do 826 nnx=1,NX
        do 826 nny=1,NY
         do 826 nnz=1,NZ
          ...
          icont=icont+1
          if(icont.gt.NX*NY*NZ/100) then
c           write(*,*) '% realizado',100*ntotal/(NX*NY*NZ)
           icont=0
          endif
          call DNS(densidad(ntotal),xi)
 826   continue
...
c        write (filename_dens,*) "dens.",nstep,".cub"
        ...
        write(189,678) NX,deltax,0.0D0,0.0D0
        write(189,678) NY,0.0D0, deltax,0.0D0
        write(189,678) NZ,0.0D0,0.0D0,deltax
        do i=1,natom
          write(189,677) Iz(i),0.0D0,r(i,1),r(i,2),r(i,3)
        enddo
        write(189,679) densidad 
        close(189)
       endif
